...
{   
    ...
    override function update(elapsed:Float) {
        ...
        if (velocity.x < 100) acceleration.x = 50;
        if (velocity.x < 250) acceleration.x = 30;
        else if (velocity.x < 400) acceleration.x = 20;
        else if (velocity.x < 600) acceleration.x = 10;
        ...
        if (_onFloor)
        {
            ...
            if (_stumble && animation.finished) _stumble = false;
            if (!_stumble)
            {
                if (velocity.x < 150) animation.play("run1");
                else if (velocity.x < 300) animation.play("run2");
                else if (velocity.x < 600) animation.play("run3");
        ...
        // Update
        if (_onFloor) velocity.y = 300;
        // just hit floor?
        super.update(elapsed);
        if (_onFloor) velocity.y = 0;
        _onFloor = false;
