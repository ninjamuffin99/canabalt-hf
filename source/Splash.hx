...
class Splash extends FlxState {
    ...
    {
        return if (step >= times.length)
                FlxColor.WHITE;
            else
                colors[(step + colorIndex) % colors.length];
    }
...
abstract LogoSprite(Sprite) to Sprite
{
    ...
    public function drawShape(shape:LogoShape):Void
    {
        ...
        switch shape
        {
            case MIDDLE: drawMiddle();
            case TOP_LEFT: drawTopLeft();
            case TOP_RIGHT: drawTopRight();
            case BOTTOM_LEFT: drawBottomLeft();
            case BOTTOM_RIGHT: drawBottomRight();
        }
